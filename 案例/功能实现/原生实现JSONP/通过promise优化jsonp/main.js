...
function getJSONP(options, callback) {
  let callbackName = options.callbackName ? options.callbackName : 'callbackName'  //  函数名其实无所谓,可以不传,只要有名字就可以
...
function changePromise(url) {
  return new Promise((resolve, reject) => {
    getJSONP({url}, data => {resolve(data)})
  })
}
...
changePromise(url1)
  .then(data => {
    console.log(data)
    return changePromise(url2)
  })
  .then(data => {
    console.log(data)
    return changePromise(url3)
  })
  .then(data => {
    console.log(data)
  })
